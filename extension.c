...
bool solve_carpark(carpark* cp, carpark* temp_cp, cp_store* s, bool print, bool animate);
...
void turns(cp_store* s, int previous, int end, bool print, bool animate);
...
void print_carpark_animated(carpark* cp);
void test(void);
...
int main(int argc, char* argv[])
{
  ...
  if(!solve_carpark(&cp, &temp_cp, &s, print, animate)){
...
bool solve_carpark(carpark* cp, carpark* temp_cp, cp_store* s, bool print, bool animate)
{
  ...
  if(carpark_check(cp)){
    ...
    if(iterations>1 && s->store[iterations].f==0){
      ...
      turns(s, s->store[iterations].f, iterations, print, animate);
      return true;
    }
  }
  return false;
...
void turns(cp_store* s, int previous, int end, bool print, bool animate)
{
  ...
  for(int i=s->moves; i>=0; i--){
    ...
    if(animate==true){
      print_carpark_animated(&s->store[solution[i]]); 
    }
  }
}
...
void print_carpark_animated(carpark* cp )
{
  ...
  for(int r=0; r<cp->y; r++){
    for(int c=0; c<cp->x; c++){
      ...
      if((cp->cp2d[r][c]>='A') && (cp->cp2d[r][c]<='Z')){
...
void test(void)
{
  ...
  cp.x=4, cp.y=4;
  str2cp(&cp, "#####AA.#..#####");
  assert(solve_carpark(&cp, &temp_cp, &s, false, false));
  assert(s.moves==1);
